...
public function balanceCheck($uid){
   $sql="SELECT sum(_amount) as total_pay  FROM _tbl_payment WHERE _status='yes' and _user_id = '$uid' group by _user_id";
   $q = $this->conn->query($sql) or die("failed!");
   $data = $q->fetch(PDO::FETCH_ASSOC);
   $totalpay=$data['total_pay'];

    $sql="SELECT sum(_total_price) as total_price  FROM _tbl_order WHERE _status='yes' and _user_id = '$uid' group by _user_id";
    $q = $this->conn->query($sql) or die("failed!");
    $data = $q->fetch(PDO::FETCH_ASSOC);
    $totalprice=$data['total_price'];

    $balance=$totalpay-$totalprice;
return $balance;
}
...
$data= array();
 $sql="SELECT * FROM $table WHERE ".$field." = '$id' order by 1 desc";
 $q = $this->conn->prepare($sql);
 $q->execute() or die(print_r($q->errorInfo()));
 while($r = $q->fetch(PDO::FETCH_ASSOC)){
 $data[]=$r;
...
{
    // retrieve the keys of the array (column titles)
 $sql="SELECT * FROM _tbl_user WHERE _email = :email AND _paw=:passwword";
 $q = $this->conn->prepare($sql);
